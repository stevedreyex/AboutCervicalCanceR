...
library(caTools)
set.seed(123)
split <- sample.split(cervical_cancer$Dx.Cancer, SplitRatio = 0.7)
train <- cervical_cancer[split, ]
test <- cervical_cancer[!split, ]
...
library(caret)
predicted <- predict(logistic_model, newdata = test, type = "response")
predicted <- ifelse(predicted > 0.5, 1, 0)
 
# Replace NA in predicted to 0
predicted[is.na(predicted)] <- 0
 
# Transform predicted to a 1D simple matrix
